...
from ..log import Exception as ZenkaiException
...
class Evaluator(loader.Module):
    ...
    class _SecureDB:
        ...
        def set(self, *args, **kwargs):
            if len(args) >= 2 and args[0] == "zenkai.security" and args[1] == "owner":
    ...
    async def e(self, message: Message):
        ...
        try:
            ...
            item = ZenkaiException.from_exc_info(*sys.exc_info())
    ...
    def censor(self, ret: str) -> str:
        ret = ret.replace(str(getattr(self._client, "tg_me", {}).get("phone", "") if hasattr(self._client, "tg_me") and isinstance(self._client.tg_me, dict) else getattr(self._client.tg_me, "phone", "") if hasattr(self._client, "tg_me") else ""), "&lt;phone&gt;")
        ...
        if btoken := self._db.get("zenkai.inline", "bot_token", False):
            ret = ret.replace(
                btoken,
                f'{btoken.split(":")[0]}:{"*" * 26}',
            )

...
class TestMod(loader.Module):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                (
                    ...
                    " Zenkai will try to determine, which client caused logs. E.g. there"
                    ...
                    " Zenkai will send all logs to Client1 and Client2, even if it is"
    ...
    def _pass_config_to_logger(self):
        ...
        handlers[0].force_send_all = self.config["force_send_all"]
        handlers[0].tg_level = {
            "ALL": 0,
            "DEBUG": 10,
            "INFO": 20,
        ...
        handlers[0].ignore_common = self.config["ignore_common"]
    ...
    @loader.command(name="debugping", aliases=["tping"])
    ...
    async def client_ready(self):
        ...
        handlers = logging.getLogger().handlers
        if not handlers or not hasattr(handlers[0], "install_tg_log"):
            logger.debug("Telegram logging handler is not available")
            return

        handlers[0].install_tg_log(self)
